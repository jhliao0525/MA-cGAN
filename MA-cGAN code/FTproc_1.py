...
def procResults():
    ...
    for i in range(130):
        ...
        loadPath = '{}/result/result-' + str(i) + '.nii.gz'.format(save_path)
        print(loadPath)
        if os.path.exists(loadPath):

            k += 1
            # args.input = save_path+'/result/result-' + str(i) + '.nii.gz'
            args.input=loadPath
            # args.output = save_path+'/result_proc/result-' + str(i) + '-proc.nii.gz'
            args.output ='{}/result/result-' + str(i) + '-proc.nii.gz'.format(save_path)
            ...
            #
            test_dice=test_dice_compare(args.output,gtStr)
