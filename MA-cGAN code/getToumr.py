...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        for ct_file, i in zip(file_list, range(Numbers)):
            ...
            new_seg = self.process(ct_path, seg_path, classes=self.classes)
            if  new_seg != None:
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        new_seg.SetSpacing((seg.GetSpacing()[0] * int(1 / self.xy_down_scale),
                            seg.GetSpacing()[1] * int(1 / self.xy_down_scale), self.slice_down_scale))
        return new_seg
...
if __name__ == '__main__':
    raw_dataset_path = './fixed_data_256_3t/'  # path of origin dataset
