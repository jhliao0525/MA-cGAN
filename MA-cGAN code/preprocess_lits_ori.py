...
import config
from ctWindow import window_transform
...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        for ct_file, i in zip(file_list, range(Numbers)):
            ...
            ct_path = self.raw_root_path + '/ct/' + ct_file
            seg_path = self.raw_root_path + '/label/' + ct_file.replace('volume', 'segmentation')
            ...
            if new_ct != None and new_seg != None:
                ...
                a = self.fixed_path + '/ct/' + ct_file
                b = ct_file.replace('volume', 'segmentation').replace('.nii', '.nii.gz')
                # sitk.WriteImage(new_ct, os.path.join(self.fixed_path, 'ct', ct_file))
                # sitk.WriteImage(new_seg, os.path.join(self.fixed_path, 'label', ct_file.replace('volume', 'segmentation').replace('.nii', '.nii.gz')))
                sitk.WriteImage(new_ct, a)
                sitk.WriteImage(new_seg, os.path.join(self.fixed_path, 'label', b))
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        ct_array = window_transform(ct_array, 170, 60)
...
if __name__ == '__main__':
    ...
    raw_dataset_path = './raw_dataset/train/'  # path of origin dataset
